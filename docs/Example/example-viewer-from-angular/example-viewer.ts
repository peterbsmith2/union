...
/**
 * - Example Viewer relies on an angular material component
 */
import { MatSnackBar } from "@angular/material/snack-bar";

/**
 * - the examples of using components are in a seperately name-spaced import
 * - there is a logical distinction between the examples source code and the component source code
 */
...
export class ExampleViewer implements OnInit {
	...
	selectCorrectTab() {
		...
		switch (fileType) {
			...
			default:
				console.error(
					`Unexpected file type: ${fileType}. Expected html, ts, or css.`
				);
		}
	}
	...
	generateUrl(file: string): string {
		...
		if (this.region) {
			...
			fileName =
				file.substring(0, last) + "-" + file.substring(last + 1) + ".html";
		}
	...
	private async _loadExampleComponent() {
		...
		this._exampleModuleFactory = new ɵNgModuleFactory(
			moduleExports[module.name]
		);
	}
...
/**
 * - The viewer is tightly coupled so that it can only show the examples from the @angular/example-components namespace
 * and by tightly coupling the examples the usage for the example-viewer is simpler, simply passing in string-based
 * keys that represent the entire example
 * - The viewer is not able to show all the files at once, only the tabbed view
 */
